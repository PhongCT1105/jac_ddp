...
- [ ] **L2. Runner registry** — allowlist mapping `job_type` → runner entrypoint; unknown types rejected with an `error` envelope.
- [ ] **L3. `noop` runner** — echoes payload; your integration-test workhorse.
- [ ] **L4. Embedding runner** — takes items from `task.json`, runs sentence-transformers (coordinate with Phong so his stub runner and your runner are the same code), writes `result.json`. Pre-downloaded weights only.
...
- [ ] **L11. `setup.sh`** — installs venv + runners + model weights to a fixed path; run it on all three Macs.
...
| With | What | When |
|---|---|---|
| Phong | Receive frozen Contract B envelopes + share the embedding runner code | End of Phong Phase 1 |
| Phong | Joint swap-in of sandbox on all workers | Phase 4 |
| Sebastian | None directly — you never see the app | — |
